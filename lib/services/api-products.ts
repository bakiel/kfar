import { Product, Vendor } from '../data/products';
...
export const productService = {
  ...
  async getProducts(params?: {
    ...
    try {
      ...
      return { 
        ...
        pagination: { total: Object.keys(products).length, hasMore: false } 
      };
    }
  },
  ...
  async getFeaturedProducts(): Promise<Product[]> {
    try {
      ...
      // Fallback to mock data
      const { products } = await import('../data/products');
      return Object.values(products).filter(p => p.isFeatured).slice(0, 6);
    }
  },
...
export const vendorService = {
  ...
  async getVendors(params?: {
    ...
    try {
      ...
      // Fallback to mock data
      const { vendors } = await import('../data/products');
      return { 
  ...
  async getVendorProducts(vendorId: string, params?: any): Promise<{ products: Product[]; pagination: any }> {
    try {
      ...
      // Fallback to mock data
      const { products } = await import('../data/products');
      const vendorProducts = Object.values(products).filter(p => p.vendorId === vendorId);
      return { 
